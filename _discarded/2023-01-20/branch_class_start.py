...
class Branch:
    ...
    def get_next_possible_steps(self):
        ...
        knn = phasing.knn(self.asteroids_kp, 0, 'orbital', T=15) # wie wähle ich t_arr und t_opt?  # visited[-1]['t_arr'] + t_opt 
        neighb_ids, neighb_dis = psa.clustering(knn, self.asteroids_kp, asteroid_2_idx)
        # Den zweiten Ast.-Idx rausschmeißen
        if asteroid_2_idx in neighb_ind: neighb_ind.pop(neighb_ind.index(asteroid_2_idx))

        print(neighb_ind) # die neighb_dis ist immer "None"
