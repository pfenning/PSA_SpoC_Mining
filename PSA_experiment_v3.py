...
for i in range(50):
    ...
    for index in neighb_inds:
        ...
        t_m_opt, t_flug_min_dv_, dv_min_ = psa.time_optimize(asteroid_1_kp, asteroid_1_mas, asteroid_1_mat,
                                                             asteroid_2_kp, t_arr=branch[i]['t_arr'], t_opt=t_opt,
                                                             propellant=bestand[-1])
        ...
        if (dv_min_/DV_per_propellant) < bestand[-1]:
            ...
            flight_opt.append([asteroid_2_id, t_m_opt, t_flug_min_dv_, dv_min_])
    ...
    # Optimum wählen:
    try:
        asteroid_2_id, t_m_opt, t_flug_min_dv_, dv_min_ = flight_opt[score_2.index(max(score_2))]
    except ValueError:
        break
    print(f"Starttag:{branch[i]['t_arr']+t_m_opt:.0f}  Flugzeit:{t_flug_min_dv_:.0f}   => Delta V ={dv_min_:.0f}")
    print(f"Ziel-Asteroid: {asteroid_2_id}  "
          f"Material:{dict_asteroids[asteroid_2_id][2]}  "
          f"Masse:{dict_asteroids[asteroid_2_id][1]:.3}")

    # Übername in Branch:
    # ToDo: Schleife Stoppen, sobald t_arr nach T_Dauer ist (was passiert mit dem  letzten Eintrag?)
    branch[i+1] = {'id': asteroid_2_id, 't_m': 0.0, 't_arr': branch[i]['t_arr'] + t_m_opt + t_flug_min_dv_}
    # t_m von aktuellem Asteroiden bestimmen
    branch[i]['t_m'] = t_m_opt
