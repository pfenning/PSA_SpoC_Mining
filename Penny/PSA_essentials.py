...
while len(visited) <= 30:  # <= 10000
    ...
    knn = phasing.knn(asteroids, ERG_t_arr[-1], 'orbital',
                      T=30)  # ACHTUNG: Referenzradius & -geschw. sind Gürtelabhängig!
    ...
    # print("Nächste Nachbarn: ", neighb_ids)
    ...
    t_abflug_opt, t_flug_min_dv, dv_min = psa.zeitoptimierung(asteroid1, asteroid2, ERG_t_arr[-1] + t_opt,
                                                              t_opt)  # Erst mal die optimale Abbauzeit = 30 Tage
...
create_submission("spoc-mining", "mine-the-belt", x, "TUDa_GoldRush_submission_file.json", "TUDa_GoldRush",
