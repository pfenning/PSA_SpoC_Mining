...
class TripInSpace:
    ...
    def plot_traj_orbits(self, ax1, schritt):
        """
        ...
        if schritt >=3:
            start_ = schritt - 3
            stop = schritt
        elif schritt >= 0 and schritt < 3:
            start_ = 0
            stop = schritt

        # Plotten der Trajektorien von Start bis Stop
        for index, el in enumerate(self.a[start_:stop], start=start_):
            # Position abrufen
            ...
            # pos and velocitiy
            r1, v1 = get_asteroid(self.a[index]).eph(t0)
            r2, v2 = get_asteroid(self.a[index+1]).eph(t1)

            # Lambert
            l = pk.lambert_problem(r1=r1, r2=r2, tof=dt, mu=MU_TRAPPIST, max_revs=2)

            # Plot planet
            plot_planet(get_asteroid(self.a[index]), t0=t0, color='r', alpha=0.1, units=AU, axes=ax1, s=5)
            plot_planet(get_asteroid(self.a[index+1]), t0=t1, color='r', alpha=0.1, units=AU, axes=ax1, s=5)
            # Plot the Lambert solutions
            axes1 = plot_lambert(l, color='b', units=AU, axes=ax1)
            # Plot part of the orbit
            plot_planet(get_asteroid(self.a[index+1]), t0=t1, tf=tv, color='k', alpha=1, units=AU, axes=ax1, s=5)
            plot_planet(get_asteroid(self.a[index+1]), t0=tv, color='y', alpha=0, units=AU, axes=ax1, s=5)

    def plot_traj_complete(self, ax1, schritt):
        """
        Plotten der Trajektorien ausgehend von Schritt start_ (also von Asteroid start_-1 auf Asteroid start_)
        bis zum Schritt stop.
        :param ax1:     Axes-Objekt, in das gezeichnet werden soll
        ...
        ax1.set_ylim(-0.3, 0.3)
        ax1.set_zlim(-0.03, 0.03)
        ax1.set_title('Alle Trajektorien')

        # Plotten der Trajektorien von Start bis Stop
        for index, el in enumerate(self.a[0:schritt],start=1):
            # Position abrufen
            t0 = pk.epoch(self.abflugzeit(index)).mjd2000 + self.t_start
            t1 = pk.epoch(self.t_arr[index]).mjd2000 + self.t_start
            tv = pk.epoch(self.abflugzeit(index+1)).mjd2000 + self.t_start
    ...
    def plot_transfer(self, ax1, schritt):
        """
        ...
        for index, el in enumerate(self.a[schritt:schritt + 1], start=schritt):
            # Position abrufen
            t0 = pk.epoch(self.abflugzeit(index)).mjd2000 + self.t_start
            t1 = pk.epoch(self.t_arr[index]).mjd2000 + self.t_start
            dt = (t1 - t0) * DAY2SEC  # muss in sec sein
    ...
    def get_score(self,index):
        return round(min(self.bestand[index][:3]),3)
