...
data = np.loadtxt("./data/spoc/mining/Asteroidengürtel.txt")
asteroids = []
for line in data:
...
class belt_mining_udp:
    ...
    def _evaluate_journey(self, x, verbose=False):
        propellant = 1  # propellant level of the ship, cannot go below 0 or we abort
        ...
        time_spent_preparing = x[n : 2 * n]  # how many days spent mining each asteroid
        material_prepared = [
            0
        ] * 4  # last is propellant and will be disregarded for score
        ...
        if verbose:
            print(f"ID\tt0\tPropellant \tDV \t  Material ID\t Prepared \t \tScore")
    ...
    def fitness(self, x, verbose=False):
        ...
        # Evaluate the journey (checks feasibility and gets the material prepared etc.)
        (
            material_prepared,
            _,
            ast_ids,
    ...
    def plot(self, x):
        """Plots the journey of the solution and prepared materials and propellant.
        ...
        (
            ...
            visited,
            propellant_at_time,
        ) = self._evaluate_journey(x, verbose=True)
        ...
        for idx in range(visited):
            # fmt:off
            plt.vlines(time_at_arrival[idx],0,material_prepared_at_t[idx][0],color="red",lw=lw)
            plt.vlines(time_at_arrival[idx] + 3,0,material_prepared_at_t[idx][1],color="green",lw=lw)
            plt.vlines(time_at_arrival[idx] + 6,0,material_prepared_at_t[idx][2],color="blue",lw=lw)
            plt.vlines(time_at_arrival[idx] - 3,0,propellant_at_time[idx],color="black",lw=lw)
            plt.hlines(-0.1,time_at_arrival[idx],time_at_arrival[idx] + time_spent_preparing[idx],color="grey",lw=6)
            # fmt:on
